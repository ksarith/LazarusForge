...
The single highest-value thing anyone could run right now: **three physically separate computers, each running a different AI agent (different model family per machine), attempting to form the quorum `Admin/Governance_Migration_Protocol.md` §VII defines, while actively working on real Forge doctrine.** The concrete protocol — machine baseline, role assignment, Git authority, non-collusion probe, pass/fail scoring — is in `Tests/Multi_Agent_Quorum_Trial.md`; log the result in `Tests/Field_Logs.md`. It will probably fail on the first attempt — log it anyway. A documented failure against the real requirements is worth more than no attempt at all.
