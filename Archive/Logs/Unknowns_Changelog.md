...
**Migration note, 2026-09-10:** v4.95 through v5.08 (14 versions) plus v4.93 (1 version, orphaned when v4.94 was migrated without it) were found stacked inline in `Unknowns.md` in violation of its own Size Management Rules — "this block now keeps only the current version." Same recurring bug class this file has caught on itself before (20 stacked at v4.84, 3 stacked at v4.87); this time the migration step lapsed for the entire 2026-09-04 through 2026-09-10 work period without being caught until James asked directly whether this changelog had been touched. All 15 versions migrated below in chronological order; `Unknowns.md` now correctly holds only v5.09. Human-prompted, Claude-executed.

**Version 5.16 — 2026-09-13. FL-006 Last Reviewed field corrected (Architecture/Forge_flow.md).** Found stale during a Claude spot-check of the Alpha_14 release: field still read 2026-09-08 despite the 2026-09-10 Risk/Priority raise (Low/Minor→Medium/Major) and the 2026-09-12 FL-007 resolution directly cross-referencing FL-006's remaining authority-ownership half as still Open. Corrected to 2026-09-12 with an inline note explaining the drift and pointing to the FL-007 cross-reference; FL-006's Open status and Resolution Path (A/B split) left untouched — this was a metadata-currency fix only, not a resolution attempt. No Open Unknowns count change. Human-directed (verification pass requested against the Alpha_14 release); Claude-executed.**

**Version 5.15 — 2026-09-12. FL-007 resolved (Reading A: Oversight Terminate is independent R4 authorization); GI-008 registered (Intake record integrity / false-clear). Prior 5.14: Grok's correction-grep results processed — verified before applying, 2 of 4 checkable claims real, 2 false.** Confirmed and fixed: SEC-007b's sidecar (`Security_Protocols.md`) and Active Index row still said "blocked pending SEC-007a" three weeks after SEC-007a's 2026-08-22 ratification — corrected both, plus Last Reviewed. Confirmed and fixed: a self-note about `Tests/Hydrologic_Resource_Cascade.md` still described its File State as declaring "Open Unknowns: 8," but that field had already been corrected to "2 Formal / 6 Unregistered" — note updated to reflect current state as historical context, not a live discrepancy. Found false: Grok's claim that Forge_flow.md's Open Unknowns count needed 13→12 (math omitted DS-001; 13 was already correct) and that Progress_Log's Forward Growth Avenues still lists GMP-006/010/GI-004/006/GU-002 as active (the section already marks all five "Resolved... No longer a Lane A candidate"). Neither false claim was actioned. Human-directed; verification requested before any fix.**
...
**Version 5.12 — 2026-09-10. ChatGPT's Phases 7-10 processed (Drift Detection, Unknown Classification, Resolution Pathway Assessment, Verification Gates) — largely stale on arrival against the prior pass's fixes, verified directly rather than assumed (including a full-file re-check confirming zero bare canonical-path violations remain anywhere). Caught a real arithmetic inconsistency in the audit's own Phase 10 scorecard (prose claimed "2/6 PASS" against a table showing 1 PASS). Genuinely new work: FL-004's stale title corrected; FL-006 Risk/Priority raised Low/Minor→Medium/Major matching FL-007's prior treatment; FL-003/FL-004/FL-006/FL-007 Resolution Paths sharpened per Phase 9's stricter closure test, FL-006 split into evidence vs authority sub-paths, FL-007 restructured into a seven-step closure sequence. G1 Fallacy Checklist and G3 Adversarial Challenge Battery formally documented for the first time — both honestly REVISE, G3 applied 3 of 10 classes with concrete scenarios, deferred the rest rather than stretching existing content to cover them. Spec Gates left at 0/6 — recording a formal tally is a governance action this pass didn't attempt. No Open Unknowns count change. Human-directed.**

**Version 5.11 — 2026-09-10. ChatGPT's multi-phase Forge_flow.md audit (Phases 1-6) batch-processed.** Six mechanical/structural fixes applied directly (missing Lessons Learned heading restored over pre-existing orphaned table content; stale Gate Correspondence Oversight row corrected to the five-exit model; FL-003's self-contradicting Description fixed; "Eight sequential decision gates" reworded to avoid the Gate-terminology collision; §1.3's bare filenames fully-qualified per Canonical_Terms.md; illustrative example values labeled non-measured). Six new Unknowns registered (FL-008 through FL-013) for genuine open architectural questions the audit surfaced, plus FL-007 strengthened (Risk Low→Medium, Priority Minor→Major) with cross-file confirmation from Gate_03_Reduction.md and Ethical_Constraints.md. One finding (FL-009) traces to a broken cross-reference the audit's own EV-008 note didn't fully surface — Forge_flow.md's Resolution Log claims a worked example exists in Gate_02_Triage.md that does not. Every claim checked against live source before action; one location error in the audit itself was caught (EV-011 misattributed a real quote to the wrong section) and corrected in the response, not silently absorbed. Human-directed.**

**Version 5.10 — 2026-09-10. Changelog migration catch-up (James asked directly whether `Unknowns_Changelog.md` had been touched — it hadn't, since v4.94 on 2026-09-03).** v4.95 through v5.09 (15 versions, plus v4.93 which had been orphaned when v4.94 migrated without it) were found stacked inline in this file in violation of its own Size Management Rules. Same recurring bug class this file has caught on itself before (20 stacked at v4.84, 3 at v4.87) — this time undetected for a full week of active work. All 16 versions migrated to `Unknowns_Changelog.md` in chronological order; this block now correctly holds only the current version again. Human-prompted, Claude-executed.**
...
**Version 4.95 — 2026-09-04. RIP-010 resolved (first Integrity Fire Drill run, mixed/informative outcome — full account in `Admin/Repository_Integrity_Protocol.md`'s own Lessons Learned and Resolution Log); RIP-012 registered from a genuine gap the drill itself surfaced (no detection mechanism for a silent, no-side-effect Resolution Log deletion). Open Unknowns for that file unchanged at 7. Human-directed.**

**Version 4.94 — 2026-09-03. Size Management Rule 2 violation fixed — same class as v4.84, this time the "Pending Corrections" cluster.** PC-001 through PC-006 (six entries) were sitting in the Active Index under `### Pending Corrections` with `Status: Resolved`, several carrying full paragraph descriptions — the rule (unchanged since v4.84's fix of the same violation on 26 other entries) requires resolved entries removed immediately, with detail living in the owning file's sidecar or, for entries with no single owning file, here in the Audit Trail. PC-005 and PC-006 already had full accounts here (v4.20, v4.24 below); PC-001–004 did not, so before removal their content is preserved: **PC-001** — Verification Ref field corrections, all 10 affected files corrected (pre-dates this changelog's 2026-07-19 split, exact date not recorded). **PC-002** — Upstream reference corrections, all 7 affected files corrected (same). **PC-003** — New file cross-reference corrections, all 10 affected files corrected (same). **PC-004** — Stale name corrections, both affected files corrected (same). None of the four had a single owning file to hold sidecar detail — all four were repo-wide reference-hygiene sweeps, which is also why the "Pending Corrections" cluster itself doesn't map to one owning file. All six removed from the Active Index and replaced with a footnote pointer, matching the existing line-245-style pattern. Found by Claude source-verifying a 2026-09-03 Grok audit of `Unknowns.md` that had specifically praised Rule 2 as "actively enforced" without catching this cluster (human-directed).

**Version 4.93 — 2026-09-01. GU-002 Resolved (Payment via Specification) — `Operations/Gate_07_Utilization.md` §5 rewritten as §5.1–5.6 (retirement handoff interface contract), with reciprocal reception doctrine added to `Operations/Gate_02_Triage.md`. Removed from Active Index, added to Resolved pointer list. Two-DS-001 hygiene preserved throughout (Gate_02's automatic-vs-operator re-triage dispute and Gate_07's fitness-for-purpose dispute both left untouched). Grok (Proposer) drafted across two revisions, ChatGPT (Skeptic) requested four amendments then confirmed Skeptic-ready, Claude (Verifier) independently caught understated tag-absent language (existing "mandatory re-triage if tag absent" rule vs. draft's discretionary "may route") and a stale Drift Indicator entry, both fixed before integration. Human-ratified though not Mandatory at Risk: Medium. Full Closure Event recorded in `Gate_07_Utilization.md`'s own GU-002 sidecar entry.**
