...
**FAK-014** — Derivation citation to `Unknowns.md` went stale twice in the same way FAK-007 already documented once: v1.15 (2026-08-30) correctly refreshed the citation to the then-current v4.87, but `Unknowns.md` advanced to v4.93 on 2026-09-01 (GU-002 closure) — two days before a 2026-09-03 Grok audit that cleared Gates 5 and 6 without catching it. The kit's own Drift Indicators explicitly name this exact condition ("Derivation statement references a superseded version of... `Unknowns.md`"; "Expiry Watch not updated at `Unknowns.md` version increment") — both were live and untripped at audit time. Surfaced by Claude, source-verifying the Grok audit against `Unknowns.md`'s own header, 2026-09-03. Context: FAK-007's "Resolved" status assumed the problem was structurally fixed by removing the hand-maintained duplicate registry at v1.10 — this recurrence shows the *citation-currency* half of that problem (distinct from the duplicate-registry half) was never actually closed, just unobserved for seven version cycles. Separately, as of 2026-09-03 the kit is no longer being used in its original run-time-condensation role — audits are being performed by loading the full repository archive and prompting directly, bypassing the token-ceiling rationale (FAK-005) that justifies the kit's existence. Whether to formally retire the kit under its own End-of-Life doctrine, or keep it current as a reference despite reduced use, is a human decision — this entry doesn't presume one. Resolution: citation refreshed to `Unknowns.md` v4.93, 2026-09-03 (human-directed). Status: Open — citation corrected; the recurring-staleness pattern itself (citation currency has no structural check, same root cause FAK-010 already named) and the kit's ongoing role are both unresolved.

**FAK-015** — Derivation citation to `Unknowns.md` went stale a third time, in exactly the pattern FAK-014 predicted when it left the "recurring-staleness pattern itself" explicitly unresolved rather than treating the v4.93 refresh as a fix. `Unknowns.md` advanced from v4.93 to v5.10 (17 versions) across 2026-09-04 through 2026-09-10 — a week of Forge_flow.md refinement work generating new Unknowns (FL-003 through FL-007, GR-009, MG-009) plus a separate changelog-migration catch-up — while this kit's citation sat untouched. Surfaced by Claude, prompted by James asking whether this file "might need a polish" immediately after fixing an unrelated `Unknowns.md`/`Unknowns_Changelog.md` migration lapse in the same conversation — not found by any audit process, scheduled check, or the kit's own Drift Indicators triggering on their own; found because a human asked. Also found in the same pass: the "Current: N open" summary line in the main file's Auditor Notes & Unknowns section had silently desynced from the File State header's Open Unknowns count — it was never updated when FAK-014 was added (stayed at "5 open" while the header correctly read 6), so it undercounted by one for the past seven days without anyone treating that as a hygiene item to check. Resolution: citation refreshed to `Unknowns.md` v5.10; "Current: N open" line corrected and both FAK-014 and FAK-015 added to its explicit list, 2026-09-10 (human-directed). Status: Open — same reasoning as FAK-014: the citation is corrected, but nothing structurally prevents a fourth recurrence. Worth naming directly rather than deferring again: this is now three occurrences of an unstructural fix to a named-but-unaddressed root cause, which is itself evidence for the retirement question FAK-014 already raised and left to human judgment — this entry still doesn't presume an answer, but the pattern is no longer merely theoretical.

**FAK-016** — Derivation citation to `Unknowns.md` went stale a fourth time, and by a wider margin than any prior occurrence: `Unknowns.md` advanced from v5.10 to v5.27 (17 versions again, but across a single five-day span of high-velocity work — the Grok provenance dispute, the two-stage scope-map passes, the skeptical-pass/application/ratification cycle, the full Gate terminology rename, and the EL-006↔SEC-007 review chain — rather than the week-plus each prior occurrence spanned. The header field itself had also silently fallen behind the file's own Resolution Log: it still read v1.16 while the log's own "Most recent" line already said v1.17, meaning FAK-015's own version bump never actually reached the header it was supposed to update — a second, smaller instance of exactly this file's own failure mode, inside the fix for the failure mode. Surfaced by an automated integrity-audit report (run against live GitHub, resumed after roughly a week paused), not by a human prompt or a scheduled internal check — a meaningfully different detection path than any of FAK-013 through FAK-015, all three of which required a human to ask. Claude independently re-verified both findings (the citation gap and the header/log mismatch) against the local tree before treating the audit's claims as fact, consistent with this session's standing discipline toward automated and agent-submitted reports. Resolution: citation refreshed to `Unknowns.md` v5.27; header corrected 1.16→1.18 in one pass (catching up the missed 1.17 bump and adding this fix); "Current: N open" line updated to 8, FAK-016 added to its explicit list, 2026-09-15 (human-directed). Status: Open — the citation is corrected, but this is now four occurrences of the same unstructural fix, and the detection path just changed from "a human happened to ask" to "an automated report happened to run" without anything about the underlying mechanism changing. The retirement/structural-fix question FAK-014 first raised, and FAK-015 restated as no-longer-theoretical, has now recurred with the recurrence's own recurrence baked into it — worth a direct human decision rather than a fifth version-string patch next time this surfaces, whenever and however it does.
