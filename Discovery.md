...
`Unknowns.md` is the most important file in the repository for understanding current system state. Open unknowns are honest acknowledgments of ignorance — suppressing them or closing them without evidence is an integrity violation. The repository maintains an Unknown Budget (floor on acknowledged unknowns) to prevent false certainty. New unknowns surfaced by honest work are welcome; premature closures are not. Closing an unknown that genuinely is resolved follows a formal process — `Admin/Auditor_Protocols.md` §Unknown Closure Authority — requiring a Proposer, an independent Verifier (never the same agent instance as the Proposer), and Human Ratification for anything touching a constitutional/meta-governance file. "Closed" is not a valid status; the real outcomes are Resolved, Discharged, Deferred, or Superseded.
...
| `Admin/Computational_Institutional_Reasoning.md` | Exploration | G4 cleared; G1 partial (Gemini 2026-06-30); G3 cleared (per `Admin/Auditor_Protocols.md`, Clear as of 2026-08-03 — corrected 2026-08-18, was previously stale here as "blocked by AP-012/AP-016") | CIR-001 — Physical Grounding Telemetry Mapping Interface undefined; \u03b31\u2013\u03b34 predicates currently aspirational in harness implementation (see Section 7.4). A candidate $S(n)$ mapping schema (v0, Draft, not ratified) was added under CIR-001 2026-08-18. Renamed 2026-07-28 from local "GOV-008" — collided with `Admin/Governance_Charter.md`'s registered GOV-008 (minimum agent quorum); CIR-scoped prefix now registered in `Admin/Forge_Audit_Kit.md`'s Governance Sidecar ID Reference. |
