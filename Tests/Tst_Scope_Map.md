...
Note: created 2026-09-18, filling a gap ChatGPT's multi-agent-readiness assessment identified — §VII specifies what a quorum *is* but nothing specified how three physical machines coordinate to attempt one. §8 Preparation Package added 2026-09-20 (Grok): recommended defaults, fillable pre-trial worksheet, six concrete non-collusion probes, Field_Logs skeleton, minimal viable first-run configuration (logical-isolation-only explicitly permitted and recorded as an honest interim per §VII.1). Status unchanged: Proposed Protocol — Not Yet Run.
