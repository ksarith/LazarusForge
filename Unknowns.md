...
**Version 5.41 — 2026-09-18. EC-012 (epistemic spoofing via hardware/firmware tampering) problem statement filed — completes the EC-011/EC-012 matched pair.** Load-bearing locations mapped: Core Mandate four-point check, EC-001/EC-002 (both now Resolved unknowns, doctrine text still live and correctly referenced), Anti-Weaponization and other hard floors, EC-008's own explicit non-resolution of EC-012. Currently-covered (EL-006 admission-time Logic-Zero wipe, non-integrable-class routing) distinguished from not-covered (ongoing in-service attestation, runtime compromise detection, any doctrine rule that unattested telemetry is untrusted for High-Risk decisions). Open questions listed: minimum attestation required, behavior on missing/failed attestation, salvaged/non-attestable-sensor honest limitation, degraded-attestation residual (same class as SEC-007b's clean-rebuild residual). Scope fenced from EC-011 (permission-source vs. telemetry compromise), EL-006/SEC-007b (admission/constitutional trust vs. ongoing sensor honesty), and EC-001/EC-002 (attestation is a precondition on their inputs, not their task). A separate procedural-rule proposal (escalate rather than treat as authoritative when High-Risk telemetry is unattested or anomalous) was raised alongside the problem statement but deliberately not filed — it would be a live doctrine addition, not a problem-statement note, and per this repository's own EC-003/EC-009/EC-008/GMP-006-008 precedent needs its own drafted sidecar, independent Skeptic review, and explicit Human Ratification before taking effect; left for a future stint. EC-012's own sidecar updated in place (Last Reviewed 2026-07-05→2026-09-18); original Description/Why It Matters/Resolution Path left intact. No attestation architecture proposed; no Payment-via-Specification; Status remains Open, Risk High, Priority Major. Cross-agent: ChatGPT drafted both the problem statement and the procedural proposal, Grok reviewed and explained the gap, Claude source-verified and filed the problem statement only, deferring the procedural addition pending ratification. Human-directed.**
