...
| AP-024 | Human attestation provenance — candidate specification integrated into `Admin/Auditor_Protocols.md` §Human Attestation Provenance Protocol 2026-08-20 (drafted by ChatGPT, five-criterion Skeptic/Evidence pass by Grok, integrated by Claude with an added reconciliation note explaining supersession of the prior 2026-07-17 H0–H5 ladder). Payment via Specification text is in place, but this file is constitutional/meta-governance, so Mandatory Human Ratification is required — not yet ratified. Verifier also pending and unusually constrained: three agents (ChatGPT, Grok, Claude) are already entangled in this specific proposal's content, so none of the three should serve as this Closure Event's Verifier per the pattern established on AP-005/AP-004 | `Admin/Auditor_Protocols.md` | Open — Pending Verification and Human Ratification | — | Major |
