...
**Version 5.26 — 2026-09-14. Gate terminology rename — residual sweep completed.** Continuation of v5.25's tranche-2 review, on James's request. Found and fixed genuine residual (current-operative text Grok's pass never touched, distinct from the anachronism-in-history bugs v5.25 already fixed): mixed old/new terminology within Claude's own FL-008 revision-note text (`Architecture/Forge_flow.md`); the historical FL-007 "(historical)"-labeled description and Resolution Path block, which had been modernized despite its own label — reverted to period-accurate "Gate D" throughout, consistent with the FL-009/v5.25 discipline; a stale cross-reference in Forge_flow.md quoting Gate_01_Intake.md's escalation-capacity heading under its pre-rename name after Gate_01 had already been updated; five live Unknown/Drift-Indicator entries in `Operations/Gate_02_Triage.md` (TS-007, TS-008, one Description, two Drift Indicators) that are current, standing doctrine, not historical narrative, and were missed; the matching two Drift Indicator rows in `Architecture/Forge_flow.md` itself; one Drift Indicator row in `Operations/Gate_03_Reduction.md`; FL-004/FL-005's current descriptions and `Operations/Tooling_Inventory.md`'s two current-doctrine mentions in `Unknowns.md`; and one each in `Architecture/Geck_forge_seed.md`, `Architecture/Components.md`, and `Architecture/Arc_Scope_Map.md` (the latter two from Claude's own FL-011-era insertions). Full final sweep confirmed every remaining old-term occurrence in live files is legitimately historical — dated Lessons Learned/Resolution Log rows, FL-012's own description of the original naming collision it's about, and one "(superseded by Closure Event)" preserved-provenance note in `Gate_07_Utilization.md` — and confirmed `Ethical_Constraints.md` (zero matches) and `Discovery.md` (already correctly renamed) needed no further action, closing out Grok's own tranche-1 follow-up list. Human-directed.
