...
**Version 5.45 — 2026-09-18. FL-006 half A: `Operations/Exception_Evidence.md` created as structural evidence-control owner for Oversight State.** Grok identified the gap precisely (HP-011 table's Oversight Operations cell blank while Gates A-D all have named owners; evidence types named in FL-006's Resolution Path have no operational home). ChatGPT's skeptical pass (REVISE not reject) made four substantive corrections before filing: (1) file ≠ operator (the file is the repository home / control spec; operational maintainer is a designated role, pending — same honesty as Tooling_Inventory / TI-001); (2) control spec ≠ warehouse (episodic packet bodies live in the records the file points to, not inside it — avoids creating another giant operational ledger); (3) evidence ≠ decision (explicit anti-policy rule: "Evidence MUST NOT be treated as a decision, authorization, routing instruction, or policy" — matters particularly once autonomous agents operate inside the Forge); (4) standing data as distributed references (Exception_Evidence.md maps what Oversight needs and where authoritative sources live, rather than centralizing scarcity/failure-rates/dependency into one file). All four accepted into the revised proposal. Verified against source before filing: HP-011 table structure, FL-006 sidecar wording, and EC-003's Recipient/Logging rows all confirmed accurate; the existing half-B partial resolution was not disturbed. HP-011's Oversight Operations cell now names the new file; FL-006 sidecar gets the half-A resolution note. FL-006 remains Open — structural owner named for half A, population and maintenance still open, half B (authority / GOV-006 dependency) untouched. Cross-agent: Grok drafted the gap analysis and initial proposal; ChatGPT ran a skeptical REVISE pass; Grok revised; Claude source-verified and filed. Human-directed.**
...
| FL-003 | Self-replication architecture ownership assigned but incomplete — `Gate_05_Separation_Thermal.md` points this to Forge_flow.md; Scope Boundary now acknowledges it (pending), but the Description had drifted into contradicting that update until fixed 2026-09-10 (ChatGPT audit, S2-002) | `Architecture/Forge_flow.md` | Open | — | Minor |
| FL-004 | Tooling inventory owned but unpopulated — not yet usable as Decision Point B evidence (title corrected 2026-09-10, ChatGPT Phase 8, from stale "has no owning file"). Closure requires population + named owner + maintenance mechanism + demonstrated usability, not population alone (Phase 9) | `Architecture/Forge_flow.md` | Open | — | Major |
| FL-005 | Decision Point B Secondary Test (justified-effort evaluation) unvalidated against real operation — new gate logic added 2026-09-08, qualitative/operator-judged with no scoring formula yet. Blocks FL-001's Decision Point B determinism claim | `Architecture/Forge_flow.md` | Open | — | Major |
| FL-006 | Oversight has no assigned Operations owner (exception-evidence gathering) or Admin owner (authority structure Escalate implicitly assumes) — found building the HP-011 transition-ownership table. **Half A (evidence): structural owner named 2026-09-18 — `Operations/Exception_Evidence.md` created as evidence-control specification and index (not a warehouse); control spec ≠ operator; anti-policy rule explicit; standing data as distributed references. HP-011 Oversight Operations cell updated. Population and demonstrated maintenance still open; half A not closed.** Half B (authority/Escalate): partial resolution stands (EC-003 Hold-safe Escalate, no new structure invented); hard limit is GOV-006 dependency (who is "on record" — authentication unresolved). FL-006 remains Open until both halves operational. | `Architecture/Forge_flow.md` | Open | — | Major |
...
| EC-011 | Human governance adversary model undefined — load-bearing assumption that human review/Human Governing Authority action is a trustworthy permission source; no doctrine for operator coercion, corruption, impairment, or captured governance. Problem statement filed (2026-09-18): assumption's load-bearing locations mapped (Ethical_Constraints.md permission model, SEC-ASM-006, Security_Protocols.md Trust Boundary, GOV-006/GMP-004 interim rule, SEC-007b R4 cells, Charter Human Override Doctrine); interim posture confirmed evidentiary/declarative-only, silent on coercion and impairment. Open questions listed (indicators, system response, verifier infinite-regress, minimum independent validation, solo-operator residual). Scope fenced from GOV-006 (authenticity of the instruction vs. trustworthiness of the authority itself — distinct questions) and EC-012 (permission-source compromise vs. telemetry spoofing — different attack surface). No adversary-model architecture proposed. | `Admin/Ethical_Constraints.md` | Open | — | Major |
| EC-012 | Epistemic spoofing via hardware/firmware tampering — entire constraint substrate treats telemetry/firmware-reported state as ground truth; no requirement that it be authenticated before being treated as authoritative. Problem statement filed (2026-09-18): load-bearing locations mapped (Core Mandate four-point check, EC-001/EC-002, Anti-Weaponization and other hard floors, EC-008's own explicit non-resolution); currently-covered (EL-006 admission-time wipe, non-integrable classes) vs. not-covered (ongoing attestation, runtime compromise detection) distinguished; open questions listed (minimum attestation required, behavior on missing/failed attestation, salvaged-sensor honest limitation, degraded-attestation residual). Scope fenced from EC-011 (permission-source vs. telemetry compromise), EL-006/SEC-007b (admission/constitutional trust vs. ongoing sensor honesty), and EC-001/EC-002 (attestation is a precondition on their inputs, not their task). Completes the matched EC-011/EC-012 pair. A separate procedural-rule proposal (escalate on unattested/anomalous High-Risk telemetry) was raised but not filed — flagged as needing its own drafted sidecar, Skeptic review, and Human Ratification per EC-003/EC-009/EC-008/GMP-006-008 precedent. No attestation architecture proposed. | `Admin/Ethical_Constraints.md` | Open | — | Major |
...
| GOV-006 | Human override authenticity validation undefined — GOV-006-A: interim authentication rules are declarative-only; zero automated resistance until Security_Protocols.md reaches Provisional Spec. Solo-operator residual (2026-09-17): with no second human operator live, the interim rule's "second human confirmation" path is practically unavailable, reducing current coverage to external cryptographic signature and dated external record only — not a rule change, a condition-of-use note; resolves automatically if GOV-007/GOV-008 resolve or a second operator appears. Scope fenced from GOV-019 (conflict arbitration between multiple authenticated overrides — distinct question) and GMP-004 (ratification-authentication twin, independently owned, not auto-resolved here). | `Admin/Governance_Charter.md` | Open | — | Major |
| GOV-007 | Bootstrap governance authority initialization — substantial Genesis Phase doctrine already exists (Q-2 separation methods, four exit pathways, Post-Exit Monitoring). Current-state consolidation filed 2026-09-18 as a structured sidecar in `Admin/Governance_Charter.md`. **Decision 1 RATIFIED 2026-09-18:** this instance is formally declared in Genesis Phase — has been since governance-bearing agent work began (May 2026); resolves the runtime-session (agent working session) vs. first-operational-run (FA-001, hot ops) ambiguity in favor of the former already being live Genesis Phase activity. G1 Resolved. **Decision 5 PARTIAL 2026-09-18:** G7 (instance boundary) reframed via `README.md`'s existing specified/demonstrated distinction — the instance's *specified* boundary (repository + Human Governing Authority + doctrine) is settled now; its *demonstrated* boundary is deliberately left open, to be earned gate-cycle by gate-cycle (starting with one melt-and-refabricate cycle) rather than declared, with a larger multi-computer/multi-agent run as a separate, higher threshold. Filed as the honest current shape of the question, not its closure — explicitly declining false certainty. No exit pathway satisfied or ratified; "first operational run" has not occurred (FA-001 Open). Remaining gaps G2-G4 fully open (pathway election, Pathway 4 horizon, standing daily posture); G6 was the sidecar itself, now filed. No new exit pathway invented, no Payment-via-Specification, no claim Genesis Phase exited. | `Admin/Governance_Charter.md` | In Progress | Active | Major |
...
| GI-008 | Intake record integrity / false-clear handling — physical chain-of-custody intact does not guarantee trustworthy entered data; candidate from ChatGPT Gate_01 audit | `Operations/Gate_01_Intake.md` | Open | — | Major |
...
| GMP-004 | Ratification authentication gap mirrors GOV-006 — interim authentication rules are declarative-only; zero automated resistance until Security_Protocols.md reaches Provisional Spec. Solo-operator residual (2026-09-18): with no second human operator live, the interim rule's "second human confirmation" path is practically unavailable, reducing current coverage to external cryptographic signature and dated external record only — not a rule change, a condition-of-use note; resolves automatically if GOV-007/GOV-008 resolve or a second operator appears. Scope fenced from GOV-006 (operational override authenticity — distinct act, independently owned by Governance_Charter.md; a shared technical solution may later serve both but does not auto-resolve either) and from GOV-019 (conflict arbitration between multiple authenticated acts — out of scope here). Precedent note retained: RIP-001 GPG-signed release tags remain the lower-friction lead for future implementation, not a resolution. | `Admin/Governance_Migration_Protocol.md` | Open | — | Major |
