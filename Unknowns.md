...
**Version 5.09 — 2026-09-10. FL-007 registered: the Gate D/Oversight authority boundary is unstated — Oversight's "Gate D want/need evaluation" is scoped to items that already passed Gate D's material-recovery determination, then potentially overrides that outcome via a different judgment axis (retained-use need) at the Terminate exit, and the text never says whether this is override authority or only timing authority. Raised by ChatGPT reviewing the 2026-09-10 working tree; verified against live text (file size, all quoted passages) before registering. Human-directed.**
**Prior: Version 5.08 — 2026-09-09. GR-009 and MG-009 registered: the provisional Reduction output envelope table (`Gate_03_Reduction.md` §4) and the provisional feedstock envelope table (`Gate_04_Separation_Mechanical.md` Inputs) — meant to mirror each other as the interim stopgap pending Reduction method selection (GR-002) — have drifted, found while scoping FL-002. Logged rather than fixed, per explicit human direction. Also fixed a real structural bug found in the same pass: `Gate_04_Separation_Mechanical.md` had its entire Resolution Log duplicated into two diverged copies (one missing a 2026-06-08 rename-fix entry and still citing a stale filename, the other missing two 2026-08 entries) — merged chronologically into one copy at the position confirmed canonical against three other files (`Gate_02_Triage.md`, `Energy.md`, `Air_Scrubber.md`). The merge was first placed at the wrong end of the file and self-corrected before finalizing. Human-directed.**
**Prior: Version 5.07 — 2026-09-09. FL-006 registered: Oversight has no assigned Operations owner (exception-evidence gathering) or Admin owner (authority structure) — found while building `Architecture/Forge_flow.md`'s §1.3 Transition Ownership table (HP-011). Also fixed a genuine content bug found in the same pass: FL-005's Resolution Path had a stray closing line that actually belonged to FL-004's topic (tooling-inventory ownership), left over from an earlier insertion. Human-directed.**
**Prior: Version 5.06 — 2026-09-08. Per-deployment clarification (human-raised): this repository is shared and forked across independent Forge builds, so localized data from any one build/session — James's own included — must never be treated as canonical or globally representative. Wording fixed in `Operations/Tooling_Inventory.md` (File Purpose, TI-001 Resolution Path), `Architecture/Forge_flow.md` (FL-004, FL-005 Resolution Paths — clarified operator-divergence checks are within-site, not cross-site uniformity), and this file's TI-001 row. No structural changes, no new invariant added — wording only, per explicit human direction to skip formalizing this as a named principle for now. Human-directed.**
**Prior: Version 5.05 — 2026-09-08. `Operations/Tooling_Inventory.md` created (Grok, from the HP-007 skeleton; Claude-reviewed — clean diff, only file touched, two File_Template.md gaps found and fixed: missing Lessons Learned and Active Disputes sections, both required even when empty). FL-004 updated to reflect the file's existence (ownership question answered, stays Open pending population). TI-001 (initial population/ownership pending) registered here, mirroring this session's practice of indexing local Unknowns globally even though the template's letter only requires it for cross-module `UNK-` entries. Components.md got a matching cross-reference stub in its own Scope Boundary. Human-directed.**
**Prior: Version 5.04 — 2026-09-08. FL-005 registered: Gate B Secondary Test (justified-effort evaluation) is new gate logic — not a rename or consolidation like this file's other 2026-09-08 additions — and has not been validated against real operation. Added same session as the Secondary Test itself, deliberately flagged more heavily as provisional than the drafting source proposed, per explicit human direction. Human-directed.**
**Prior: Version 5.03 — 2026-09-08. FL-004 registered: tooling inventory (ASM-003) has no owning file — `Architecture/Components.md` checked as candidate and confirmed not to claim this role (system taxonomy, not a tool list). Found while reviewing a Grok-drafted `Operations/Tooling_Inventory.md` proposal; the gap was verified against source before registering, the drafted fix logged separately as HP-007 in Forge_flow.md's Held Proposals (not yet created). Human-directed.**
**Prior: Version 5.02 — 2026-09-08. HP-006 cross-layer reconciliation pass (Forge_flow.md Held Proposals) — checked every "DOES NOT define → see X" Scope Boundary pointer across Forge_flow.md, Gate_02/03/04/05, Energy.md, and Forge_Net.md against the receiving file's actual body/Scope content. Four orphaned handoffs found (term coined/pointed-to in one file, never picked up in the receiving file): TS-009 (Unknown Bulk), TS-010 (battery chemistry sorting), SC-010 (Class C terminology), FL-003 (self-replication architecture). All four registered here and in their owning file's own sidecar; one-line stub acknowledgments added to each receiving file's Scope Boundary. No contradictions found — only orphaned handoffs; most cross-references reconciled cleanly. Human-directed.**
**Prior: Version 5.01 — 2026-09-07. Size Management Rule 2 hygiene fix: RIP-003 and RIP-010 had been left in the Active Index with updated "Resolved" status text rather than removed at closure time (2026-09-03 and 2026-09-04 respectively) — same violation class as the Pending Corrections cluster fixed at v4.94. Both removed with a footnote pointer. Caught by a Grok morning-report sweep, verified by Claude against source before fixing. Human-directed.**
**Prior: Version 5.00 — 2026-09-06. New file `Architecture/Local_Governance_Framework.md` (v0.1-draft) adopted, drafted by Grok following a cross-cutting observation surfaced during the Challenges/ audit sweep. LGF-001 through LGF-007 registered. All five referenced Challenges cross-IDs (WS-004, WA-003, PO-004, ES-001, ES-002) verified against source before adoption. Human-directed.**
**Prior: Version 4.99 — 2026-09-05. RIP-011 and RIP-012 jointly Ratified via §Lightweight Revision Anchors and Deletion Detection in `Admin/Repository_Integrity_Protocol.md` (drafted by Grok; Claude's Skeptic pass caught and Grok fixed a real scope-boundary issue — the check had been mislabeled as amending Gate 5, which is owned by `Admin/Verification_Gates.md`, not RIP; corrected to RIP's own Phase 1 vocabulary; Human Governing Authority ratified same day). Removed from Active Index per Rule 2. `Admin/Repository_Integrity_Protocol.md` Open Unknowns 7→5. Human-directed.**
**Prior: Version 4.98 — 2026-09-05. GMP-008 Ratified via §III.D Expiration of Stalled Pre-Recorded Track B Proposals (drafted by Grok, independently Skeptic-reviewed by Claude against live §III.A/original-Resolution-Path source, Human Governing Authority ratified same day). Removed from Active Index per Rule 2. `Admin/Governance_Migration_Protocol.md` Open Unknowns 6→5. The GMP-006/007/008 amendment-lifecycle cluster is now fully Ratified. Human-directed.**
**Prior: Version 4.97 — 2026-09-05. GMP-007 Ratified via §III.C Withdrawal of Pre-Recorded Track B Proposals (drafted by Grok, independently Skeptic-reviewed by Claude against live §III.A source, Human Governing Authority ratified same day). Removed from Active Index per Rule 2. `Admin/Governance_Migration_Protocol.md` Open Unknowns 7→6. Human-directed.**
**Prior: Version 4.96 — 2026-09-05. GMP-012 Ratified via §III.B Repeal and Rollback of Recorded Track B Amendments (drafted by Grok, independently Skeptic-reviewed by Claude against live source, Human Governing Authority ratified same day). Removed from Active Index per Rule 2. `Admin/Governance_Migration_Protocol.md` Open Unknowns 8→7. Human-directed.**
**Prior: Version 4.95 — 2026-09-04. RIP-010 resolved (first Integrity Fire Drill run, mixed/informative outcome — full account in `Admin/Repository_Integrity_Protocol.md`'s own Lessons Learned and Resolution Log); RIP-012 registered from a genuine gap the drill itself surfaced (no detection mechanism for a silent, no-side-effect Resolution Log deletion). Open Unknowns for that file unchanged at 7. Human-directed.**
**Prior: Version 4.94 — 2026-09-03. Size Management Rule 2 violation fixed — "Pending Corrections" cluster (PC-001–006, six entries) was sitting in the Active Index with Status: Resolved, several with full descriptions, in violation of the rule immediately below. All six removed; detail preserved in `Archive/Logs/Unknowns_Changelog.md` (v4.94) since none had a single owning file. Found by Claude source-verifying a same-day Grok audit (human-directed). Full account in the changelog.**
**Prior: Version 4.93 — 2026-09-01. GU-002 Resolved (Payment via Specification) — `Operations/Gate_07_Utilization.md` §5 rewritten as §5.1–5.6 (retirement handoff interface contract), with reciprocal reception doctrine added to `Operations/Gate_02_Triage.md`. Removed from Active Index, added to Resolved pointer list. Two-DS-001 hygiene preserved throughout (Gate_02's automatic-vs-operator re-triage dispute and Gate_07's fitness-for-purpose dispute both left untouched). Grok (Proposer) drafted across two revisions, ChatGPT (Skeptic) requested four amendments then confirmed Skeptic-ready, Claude (Verifier) independently caught understated tag-absent language (existing "mandatory re-triage if tag absent" rule vs. draft's discretionary "may route") and a stale Drift Indicator entry, both fixed before integration. Human-ratified though not Mandatory at Risk: Medium. Full Closure Event recorded in `Gate_07_Utilization.md`'s own GU-002 sidecar entry.**
...
| FL-002 | Reduction output envelope cross-validation pending — `Operations/Gate_03_Reduction.md` exists (was previously missing; corrected 2026-08-18) and specifies constraints-first Reduction doctrine, but its output envelope is not yet cross-validated against `Operations/Gate_04_Separation_Mechanical.md`'s provisional feedstock envelope | `Architecture/Forge_flow.md` | Open | — | Major |
...
| TS-009 | "Unknown Bulk" review criteria undefined — Gate_04 points human review of its Unknown Bulk output classification to this file, which never uses the term | `Operations/Gate_02_Triage.md` | Open | — | Major |
| TS-010 | Battery chemistry sorting algorithms undefined — Energy.md defers this to this file, which contains no chemistry-sorting content | `Operations/Gate_02_Triage.md` | Open | — | Minor |
| SC-010 | "Class C" terminology not reconciled — Gate_04's Class A/B/C/Unknown Bulk/Fail classification points Class C to `Gate_05_Separation_Thermal.md`, which never uses the term | `Operations/Gate_05_Separation_Thermal.md` | Open | — | Minor |
| FL-003 | Self-replication architecture ownership undefined — `Gate_05_Separation_Thermal.md` points this to Forge_flow.md, which never claims it | `Architecture/Forge_flow.md` | Open | — | Minor |
| FL-004 | Tooling inventory (ASM-003) has no owning file — checked `Architecture/Components.md` as candidate, it's a system taxonomy not a tool list and doesn't claim this ownership. **Update 2026-09-08:** `Operations/Tooling_Inventory.md` created (HP-007 done); ownership question answered, entry stays Open pending first population (TI-001) | `Architecture/Forge_flow.md` | Open | — | Major |
| FL-005 | Gate B Secondary Test (justified-effort evaluation) unvalidated against real operation — new gate logic added 2026-09-08, qualitative/operator-judged with no scoring formula yet. Blocks FL-001's Gate B determinism claim | `Architecture/Forge_flow.md` | Open | — | Major |
| FL-006 | Oversight has no assigned Operations owner (exception-evidence gathering) or Admin owner (authority structure Escalate implicitly assumes) — found building the HP-011 transition-ownership table; Gates A-D all have clear owners, Oversight does not | `Architecture/Forge_flow.md` | Open | — | Minor |
| FL-007 | Gate D/Oversight authority boundary unstated — Oversight's "Gate D want/need evaluation" can override an item that already passed Gate D's material-recovery determination via a different judgment axis (retained-use need), and the text never says whether Oversight has override authority or only timing authority. Raised by ChatGPT, verified against live text | `Architecture/Forge_flow.md` | Open | — | Minor |
| TI-001 | Initial population and ownership assignment pending for the live tooling inventory (per-deployment — this repo is shared across independent Forge builds, so no single population closes this globally). Tables are structural placeholders, no physical count performed, no named owner | `Operations/Tooling_Inventory.md` | Open | — | Major |
...
| GOV-008 | Minimum hardware and agent quorum for bootstrap compliance — candidate specification drafted 2026-07-31 (`Admin/Governance_Migration_Protocol.md` §VII), explicitly held to the governance-independence bar §VI's own EQD Non-goal clause distinguishes from epistemic independence; extended 2026-08-06 with §VII.8 (registry/runtime-gate reconciliation) after an independent thread's competing Charter-level draft was rejected in favor of this entry's existing owner; not ratified, does not resolve this entry. Ratifiability audit 2026-08-18 (prompted by a `CIR_Gov.md` ratification attempt) confirmed the candidate's content is sound end-to-end, but §VII.6's Hardware/Runtime Diversity requirement is "declarable, not achieved" — no second physical host exists in this repository's actual operating environment. Not a documentation gap; blocked on physical infrastructure | `Admin/Governance_Charter.md` | Open | — | Major |
...
| CIR-GOV-001 | Predicate-to-Checkpoint mapping incomplete — candidate mapping table supplied 2026-07-31, two genuine gaps documented at Checkpoints 2 (Structural Plausibility) and 4 (Cross-Module Integration), zero predicate coverage on either. File itself is Proposed — Not Ratified. | `Admin/CIR_Gov.md` | Open | — | Low |
...
| CIR-001 | Physical Grounding Telemetry Mapping Interface undefined — γ1–γ4 predicates aspirational pending harness Phase 2; renamed 2026-07-28 from a colliding local "GOV-008" (see `Admin/Forge_Audit_Kit.md`'s Governance Sidecar ID Reference); registration gap closed 2026-07-29 — this tracker existed in `Admin/Computational_Institutional_Reasoning.md`'s own sidecar since the rename but was never mirrored here, despite its sibling tracker CF-004 (same file's Active Trackers list) being registered. A candidate $S(n)$ mapping schema (v0, Draft, not ratified) was added under this tracker 2026-08-18 — remains Open until a first concrete Leviathan log → $S$ derivation is demonstrated against it | `Admin/Computational_Institutional_Reasoning.md` | Open | — | Critical |
| RIP-002 | AUDIT_HARNESS.py Phase 1 checks partially implemented (3 of 6 live: Ethical Anchor, File State, cross-reference; FROZEN/nav/parity outstanding) | `Admin/Repository_Integrity_Protocol.md` | Open | — | Major |
*RIP-003 (violation incident log location) Resolved 2026-09-03 via `Integrity_Incident_Log.md`; RIP-010 (Integrity Fire Drill) Resolved 2026-09-04 — both removed from the Active Index per Size Management Rule 2. Full detail in `Admin/Repository_Integrity_Protocol.md`'s own sidecar and Resolution Log. (Both had been left in this table with an updated "Resolved" status string rather than removed at the time each closed — a Rule 2 violation of the same class as the Pending Corrections cluster fixed 2026-09-03, caught by a Grok morning-report sweep 2026-09-07.)*
...
*RIP-011 (§Version Preservation lightweight anchor rule) and RIP-012 (silent Resolution Log deletion detection) both Ratified 2026-09-05 via joint §Lightweight Revision Anchors and Deletion Detection — removed from active index per Size Management Rule 2. Full detail in `Admin/Repository_Integrity_Protocol.md`'s own sidecar entries.*
...
*The following are Resolved and do not appear in this active index per Size Management Rule 2 — full closure detail is in `Archive/Logs/Auditor_Protocols_Logs.md`'s own sidecar entries: AP-004 (2026-08-20), AP-005 (2026-08-19), AP-013 (2026-08-19), AP-024 (2026-08-20). See Audit Trail in `Archive/Logs/Unknowns_Changelog.md` for the brief resolution note on each.*
...
### Local Governance (new 2026-09-06)

| ID | Title | Owning File | Status | Subtype | Priority (Promo) |
|---|---|---|---|---|---|
| LGF-001 | Minimum viable host-community standing definition — what concrete acts or recognitions constitute standing for the decision classes this framework names | `Architecture/Local_Governance_Framework.md` | Open | — | Major |
| LGF-002 | Conflict-resolution path when node technical constraints and community preferences cannot be reconciled inside hard floors | `Architecture/Local_Governance_Framework.md` | Open | — | Major |
| LGF-003 | Exit and non-dependency standards — measurable criteria that an arrangement has not created structural dependence | `Architecture/Local_Governance_Framework.md` | Open | — | Major |
| LGF-004 | Interface doctrine with pre-existing informal and formal local institutions (extends WA-003) | `Architecture/Local_Governance_Framework.md` | Open | — | Major |
| LGF-005 | Record format for local disposition decisions so they remain legible and revisable across operator changes | `Architecture/Local_Governance_Framework.md` | Open | — | Minor |
| LGF-006 | Boundary cases — multi-node deployments, transient/non-geographic communities, no identifiable host community | `Architecture/Local_Governance_Framework.md` | Open | — | Major |
| LGF-007 | Relationship to multi-node or federated structures — does local standing survive upward coordination? | `Architecture/Local_Governance_Framework.md` | Open | — | Major |

*Registered 2026-09-06 with the file's v0.1-draft creation. Supplies the institutional interface layer that `Challenges/Water.md` WS-004, `Challenges/Waste.md` WA-003, `Challenges/Planned_Obsolescence.md` PO-004, and `Challenges/Energy_Scarcity.md` ES-001/ES-002 had each been gesturing at independently without a shared owner — all five cross-refs verified against source before this file was adopted (Claude, human-directed). Does not close any of those five; supplies the standing/decision-rights/reversibility requirements they should reference going forward. Surfaced from a cross-cutting observation during the Challenges/ audit sweep: the "generate surplus, then share it" framing recurring across those files was an unexamined ideological default, not a derived engineering necessity — this framework keeps the disposition choice itself local and revisable rather than picking one.
...
*WA-002 and WA-004 are Resolved and do not appear in this active index per Size Management Rule 2 — full closure detail is in `Challenges/Waste.md`'s own Resolution Log (WA-002: 2026-08-23, Payment via Specification; WA-004: 2026-08-24, Discharge via Consolidation — the same disposal doctrine as `Operations/Gate_03_Reduction.md` GR-003, tracked from the Challenges/ side, not a second problem). See Audit Trail in `Archive/Logs/Unknowns_Changelog.md` for the brief resolution note on each.*
...
*Phase 1 Resolution Pass (2026-07-30) closed SD-UNK-008/009/010/012/013/014/015 with logged pathways (Trajectory/Specification/Vehicle, per `Tests/Solar_Descent.md` Resolution Log) and demoted SD-UNK-003/007/011 to Minor. Open set: 001, 002, 003, 004, 005, 006, 007, 011 (8 total) — matches `Tests/Solar_Descent.md` File State exactly. The seven Resolved entries removed from this active index per Size Management Rule 2, 2026-08-24 — see Audit Trail in `Archive/Logs/Unknowns_Changelog.md`.*
...
| MG-009 | Provisional feedstock envelope table drifted from Gate_03_Reduction.md's mirror table — found scoping FL-002; logged rather than fixed since both are placeholders pending Reduction method selection | `Operations/Gate_04_Separation_Mechanical.md` | Open | — | Minor |
...
| GR-009 | Provisional output envelope table drifted from Gate_04_Separation_Mechanical.md's mirror table — found scoping FL-002; logged rather than fixed since both are placeholders pending Reduction method selection | `Operations/Gate_03_Reduction.md` | Open | — | Minor |

*GR-003 is Resolved and does not appear in this active index per Size Management Rule 2 — full closure detail is in `Operations/Gate_03_Reduction.md`'s own Resolution Log (2026-08-24). See Audit Trail in `Archive/Logs/Unknowns_Changelog.md` for the brief resolution note.*
...
*PL-002 is Blocking before any reactor fabrication. PL-001 is Resolved and does not appear in this active index per Size Management Rule 2 — full closure detail is in `Operations/Plastics.md`'s own Resolution Log (2026-08-23); Blocking Yes for hot pyrolysis runs remains in force via PL-001-R1 despite specification closure. See Audit Trail in `Archive/Logs/Unknowns_Changelog.md` for the brief resolution note.*
...
*All entries in this cluster (PC-001 through PC-006) are Resolved and do not appear in this active index per Size Management Rule 2. None had a single owning file — all were repo-wide reference-hygiene sweeps — so full detail is preserved in `Archive/Logs/Unknowns_Changelog.md` (v4.94) rather than an owning-file sidecar. Fixed 2026-09-03 after sitting in the Active Index in violation of Rule 2 since before this changelog's 2026-07-19 split (PC-001–004) or since 2026-07-12/07-19 (PC-005/006).*
...
| PYC-001 | Halogenated-polymer triage dependency — blocks all hot thermal work under this file. PL-001 Resolved 2026-08-23; CE-003 remains Open — one of two dependencies cleared | `Tests/Pyrolysis_Cascade.md` | Open | — | Critical |
...
| PYC-003 | Hazardous-fraction and disposal dependency (WA-002, GR-003, WA-004 — all three Resolved 2026-08-23/24); Blocking remains Yes on WA-002-R1/GR-003-R1 empirical and jurisdiction residuals, not on missing doctrine | `Tests/Pyrolysis_Cascade.md` | Open | — | Critical |
...
| SEC-007b | External root-of-trust, physical implementation layer — SEC-007a (constitutional layer) Resolved 2026-08-22; SEC-007b now the sole remaining blocker, not "pending SEC-007a" |
| UNK-009 | External root-of-trust cross-module — spans GOV-005, RIP-001, SEC-007a/SEC-007b (GOV-003 Resolved 2026-08-23, no longer a live span component) |
...
| EM-004 | Governance substrate integrity under emergent agent access — Critical; broader than GOV-003/SEC-007a alone (both Resolved 2026-08-22/23) — this unknown's own scope remains open |
