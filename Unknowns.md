...
**Version 4.27 — 2026-07-27. Registration-latency sweep across three files (human-directed, prompted by a direct question rather than a scheduled audit). Five entries registered in their owning file's sidecar over the prior four days had never been cross-indexed here: GOV-022 (`Admin/Governance_Charter.md`, reversibility operating-principle placement), RIP-010 (`Admin/Repository_Integrity_Protocol.md`, Integrity Fire Drill), GMP-011/GMP-012/GMP-013 (`Admin/Governance_Migration_Protocol.md`, dispute resolution / rollback doctrine / EQD tooling gap). A deeper, older gap surfaced in the same sweep: GOV-021 has never been formally registered anywhere — `Admin/Autonomy_Divergence_Protocol.md` reserved it as a candidate ID on 2026-07-19 and explicitly flagged that file as outside the current paste-based edit workflow, so registration in `Admin/Governance_Charter.md`'s own sidecar never happened. Added here as a Vehicle-subtype entry pointing at that gap directly, since the registering file cannot itself be edited under the current workflow. Open Unknowns count in `Admin/Governance_Charter.md`: 19 → 20 (GOV-021 newly registered; GOV-022 was already reflected in that file's own count). Open Unknowns count in `Admin/Repository_Integrity_Protocol.md`: 8 → 8 (RIP-010 was already reflected in that file's own header; this was a cross-index gap only, not a header discrepancy). Open Unknowns count in `Admin/Governance_Migration_Protocol.md`: 10 → 10 (same — header already correct, cross-index gap only).**
...
*Versions 4.16–4.22 (compressed; full text in `Unknowns_Changelog.md`):*
- **v4.22** (2026-07-17): Three ratification-only items adopted — ENV-DS-001 ratified; Embedded Value Preservation ratified into CLF-004's Scope Boundary; CLF-004's chlor-alkali pathway reframed as one of three options under consideration, CE-006 gained a directional (not completed) note.
