...
**Version 5.36 — 2026-09-17. SEC-007b transcript extended (Part 6) — R1–R6 matrix filled for all five candidates; Candidate 1's Conditional cells deepened into numbered open questions.** No candidate earns an unqualified Pass across all six requirements. Candidate 5 (vendor secure-boot locked MCUs) eliminated for the constitutional-anchor role — confirmed against live Electronics.md non-integrable-class doctrine before accepting. Candidates 1 (offline signed snapshot + GPG) and 2 (EEPROM/OTP write-once) are the leading hypotheses, both Conditional on R5 and parts of R2/R6 for the exact reasons Part 5's checklist surfaced. Candidate 1's four Conditional cells (R2, R3, R5, R6) expanded into concrete, individually-numbered design questions rather than left as vague "needs more work" notes. **New finding, flagged not resolved:** every cell touching "operator set," "dual-control," or "human-ratified" is quietly assuming the legitimate-human-operator question is settled — it isn't. GOV-006 (override authenticity validation) is real and still Open; `Admin/Governance_Charter.md` explicitly states it "must not be implicitly assumed," verified directly. Same category of dependency FL-006's authority half had to surface and cross-reference rather than silently assume. Filed as a soft dependency for whichever candidate's ceremony design gets picked up next, not as a blocker to the matrix work itself. Appended as Part 6 to the existing transcript, cross-referenced from SEC-007b's own sidecar entry. No candidate selected; SEC-007b's Status (Open) and PAT-002's lineage (Decision Drafted) unchanged. Human-directed.
