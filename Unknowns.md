...
**Version 5.06 — 2026-09-08. Per-deployment clarification (human-raised): this repository is shared and forked across independent Forge builds, so localized data from any one build/session — James's own included — must never be treated as canonical or globally representative. Wording fixed in `Operations/Tooling_Inventory.md` (File Purpose, TI-001 Resolution Path), `Architecture/Forge_flow.md` (FL-004, FL-005 Resolution Paths — clarified operator-divergence checks are within-site, not cross-site uniformity), and this file's TI-001 row. No structural changes, no new invariant added — wording only, per explicit human direction to skip formalizing this as a named principle for now. Human-directed.**
