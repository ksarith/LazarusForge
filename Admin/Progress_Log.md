...
- **GMP-006** (Concurrent amendment handling) — Open, Major. Resolution Path is a clean, self-contained state-machine definition task; no upstream dependency. Confirmed clean.
...
- **GMP-010** (Evidence-sufficiency gate) — Open, Major, **High risk**. Highest-leverage item in this batch — directly targets the CE-006 near-miss failure mode (a directed approach treated as settled before verification). Resolution Path is a definition task (a lightweight gate, not full Track A/B process); no upstream dependency. Confirmed clean.
...
- **GI-004** (Intake tagging schema ↔ grain system) — Open, Major. Resolution Path is pure cross-validation (compare two field sets, extend as needed); no upstream dependency. Grok path-adequacy reviewed 2026-08-14. Resolves jointly with GI-006 per both files' own cross-reference.
- **GI-006** (Intake chain-of-custody) — Open, Major. Same pattern as GI-004 — doctrine definition, no upstream dependency, Grok-reviewed 2026-08-14, joint resolution with GI-004.
...
1. Lane A repopulated 2026-08-30 — 7 confirmed candidates across two clusters (GMP amendment lifecycle; Intake/Utilization interface contracts), plus 2 split spec/Automation items. See Lane A section above. Recommended first pick: **GMP-010** (evidence-sufficiency gate) — highest risk-rated item in the batch and the one both agent notes flagged as directly preventing a recurrence of the CE-006 near-miss.
