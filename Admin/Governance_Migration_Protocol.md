...
**Role rotation across cycles [Added 2026-09-18, human-directed]:** an agent instance may hold different classes in different quorum cycles (e.g., Skeptic in cycle N, Planner in cycle N+1). This is **permitted, not required**, and is scoped narrowly:

- *Within* any single quorum cycle, the existing rule is unchanged and absolute: no evaluator holds more than one role, per VII.3's operational definition. Rotation is a cross-cycle property only; it does not relax anything within-cycle.
- *Rationale, not a requirement:* a fixed Planner/Skeptic pairing risks developing a stable rhythm over repeated cycles — the Skeptic implicitly learning what the Planner tends to produce and scrutinizing it less. Rotation is one candidate mitigation for that drift, consistent with the intent behind VII.4's Non-collusion test, but is not itself a substitute for that test or for the Diversity/Independence tests, which still apply in full to whatever the current cycle's assignment is.
- **What rotation does not solve, stated plainly:**
  - It does not satisfy the Diversity or Independence tests by itself — if the same small pool of model instances simply relabel which class they hold, VII.4's "vendor label is necessary but not sufficient" clause applies exactly as before to the new assignment.
  - It does not touch the Hardware/Runtime Diversity requirement (VII.1) — rotating roles across the same physical hosts does not create the physical diversity that requirement demands.
  - It does not by itself detect collusion — a Non-collusion test that would catch two rotating agents converging on shared framing over time is not specified here and remains an open item (see VII.6).
  - **Assignment authority is unresolved and deliberately left open, not decided here:** who or what determines a given cycle's rotation (a fixed schedule, a human decision each cycle, an agent self-selecting) is not specified. A rotation scheme where one party effectively controls every cycle's assignment could quietly reintroduce a single point of control this taxonomy exists to prevent — that risk is named here so it is not silently inherited by whatever rotation mechanism is eventually built.
- No claim that specifying rotation moves GOV-008 closer to Resolved, or that this addition resolves any part of VII.6's Open Items list. Consistent with this entire section's status: Candidate specification, Payment via Specification only.
...
- **Rotation assignment authority [Added 2026-09-18]:** if cross-cycle role rotation (VII.2) is used, who or what determines each cycle's assignment — a fixed schedule set in advance, a human decision each cycle, or agent self-selection — and whether that assignment mechanism itself needs an independence requirement, is undecided. Left explicitly open rather than defaulted.
