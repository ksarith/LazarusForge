...
| Last Audit       | 2026-07-31; CE-006 Resolution Path refreshed 2026-08-16 (`Admin/Resolution_Methodology.md` third applied case) — file-level Last Audit not bumped at the time; corrected 2026-08-30 |
...
**Resolution Path (refreshed 2026-08-16 under `Admin/Resolution_Methodology.md`):** Mechanism is Stage D caustic (NaOH) wet scrubbing — not Stage E — with capture-and-nullification as the directed approach (2026-07-17/19). Paper-complete doctrine now covers: stoichiometry and operating-parameter table; detection/alarm thresholds (NIOSH/OSHA/EPA AEGL/AIHA ERPG-sourced); fail-safe interlock (DC supply hard series trip through Stage D recirculation/power monitoring); Faraday-law Cl₂ generation-rate formula (mass rate as function of current); CE-006 own reaction-exotherm formula; membrane/diaphragm candidate classes (salvage-first ceramic diaphragm vs. external-dependency cation-exchange membrane); CE-007 storage/stability/reuse doctrine. **Remaining equipment-gated work (honest §4 split):** (1) sealed reaction vessel physical design + build confirming no anode-side atmospheric leak path before the scrubber stage; (2) electrode-area / specific-current selection that turns the Faraday formula into a first-article number, plus AS-003 calibration against real flow; (3) combined thermal-sink number (CE-006 exotherm + Stage D base load) — blocked on `Gate_05_Separation_Thermal.md` exhaust characterization; (4) quantitative NaOCl storage sizing and membrane final selection/sourcing. Residual NaOCl that cannot be reused routes under `Operations/Gate_03_Reduction.md` GR-003's two-outcome disposal model (passive containment only if structurally bound and intact; otherwise external specialist hand-off). Cross-reference back to `Challenges/Closed_Loop_Feedstock.md` CLF-004 once the equipment-gated items close. Payment via Specification only after sealed-vessel + AS-003 calibration data exist — not on further paper work alone.
...
**Resolution_Methodology application, 2026-08-16 (third applied case — chemistry/safety domain):**

*§1 (dependency-hollowness):* AS-003 remains Open (calibration). `Gate_05_Separation_Thermal.md` exhaust heat load remains Placeholder. CE-007 quantitative sizing correctly still blocked on this unknown's hardware gap. CLF-004 correctly still blocked on CE-006. GR-003 now has real two-outcome disposal doctrine (written under the same methodology, 2026-08-15) — previously a hollow cross-ref for residual NaOCl that cannot be reused; now a named destination.

*§2 (reuse-before-invent):* No new scrubber, interlock sensor class, or Waste_Handling file invented. Stage D caustic path, Stage D flow/power monitoring as interlock trigger, and GR-003 disposal model are all existing named mechanisms. Residual NaOCl liquor that fails CE-007 reuse criteria routes under GR-003 rather than a parallel path.

*§3 (verify-before-accept):* Prior Resolution Path still listed items (2) caustic dosing / residence and (3) detection/alarm thresholds as "remaining work" even though both had doctrinal answers since 2026-07-31 and the 2026-08-15 pass. Refreshed above to match body reality. Faraday formula, exotherm formula, and interlock series-trip language verified present in body before claiming paper-complete.

*§4 (digital/equipment-gated split):* Paper surface on CE-006 is now substantially exhausted. Remaining work is equipment-gated by nature (vessel build, electrode-area selection, AS-003 calibration against real flow, Gate_05 exhaust data, quantitative NaOCl sizing, membrane sourcing/test). Further paper passes without hardware will not move Status toward Resolved. Stays **In Progress**.

*§5 (scope-map stale-pointer):* `Architecture/Arc_Scope_Map.md` last reviewed earlier than this pass; Chemistry.md entry should be checked for ownership/Does-not drift on next Architecture folder refresh. No immediate Scope Boundary contradiction found in this file itself.

CE-006 remains **In Progress** — Blocking Yes for CLF-004 candidate pathway unchanged. No claim of Resolved.
...
- 2026-08-16: **CE-006 — Resolution_Methodology third applied case (chemistry/safety domain).**
